...
emerge as dominant SHAP drivers. A sensitivity analysis confirms strong convergence of macro
...
\textbf{Granger Causality and ARDL.} Tests use Bonferroni-corrected
$\alpha{=}0.01$ (5 predictors, maxlag=1). No predictor clears the corrected
threshold at $n{=}10$. Youth LFPR is suggestive at nominal $\alpha{=}0.10$
($p{=}0.082$). Due to the $n{=}10$ constraint, Johansen cointegration
(singular matrix) and full ARDL (12 regressors ${>}$ 9 d.f.) remain
infeasible on the annual dataset. The target ARDL($p,q$) specification for
...
coefficients to be tested via Pesaran's $F$-bounds
procedure~\cite{pesaran2001}. Estimation of Eq.~\eqref{eq:ardl} is
deferred to the full quarterly dataset ($n{\approx}43$). A fallback OLS
with HC3 robust standard errors is estimated in the interim
($R^2{=}0.767$, adj.~$R^2{=}0.475$), with diagnostics confirming
well-behaved residuals (Durbin-Watson: 2.70; Breusch-Pagan: $p{=}0.14$;
Jarque-Bera: $p{=}0.84$).
...
confirms structural amplification of GDP elasticity post-2022. The
sensitivity analysis confirms SHAP rankings are strongly convergent across
...
\textbf{Limitations.} The annual dataset ($n{=}10$) rendered full ARDL
(Eq.~\ref{eq:ardl}) and Johansen infeasible; results are directional
pending quarterly expansion. The 2022 LFS artefact complicates annual
inference for the hours-based series. Agricultural data ends at 2024;
FAOSTAT 2025 crop indices are not yet released. The XGBoost model achieves
limited out-of-sample fit at $n{=}10$, and SHAP values should be
interpreted as association measures, not causal estimates. The gender
Zivot-Andrews test lacks power at $n = 10$; quarterly gender data would
strengthen break detection.
...
(ii) incorporate the 2025 annual LFS data upon release; (iii) extend
gender analysis to quarterly disaggregation for higher-power break
detection; and (iv) develop district-level panel models to explain
spatial heterogeneity beyond informality.
