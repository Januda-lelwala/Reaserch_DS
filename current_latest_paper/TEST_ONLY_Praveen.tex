...
\textbf{Multicollinearity.} Variance Inflation Factors are computed prior to long-run modelling using:
%
\begin{equation}
...
\end{equation}
%
where $R_j^2$ is the coefficient of determination from regressing predictor $j$ on all other predictors (Table~\ref{tab:vif}). Exchange Rate registers $\mathrm{VIF}{=}12.28$, exceeding the threshold of 10, driven by near-collinearity with Inflation during the 2022 crisis. Exchange Rate is therefore excluded from the ARDL model; its effect is captured indirectly through Inflation. All remaining predictors have $\mathrm{VIF}{<}8$.
...
\textbf{Structural Breaks.} The Zivot-Andrews Model~C test~\cite{zivot1992} fits the regression:
%
\begin{equation}
  \Delta y_t = \mu + \beta t + \gamma\, DU_t(\lambda) + \delta\, DT_t(\lambda)
              + \alpha y_{t-1} + \sum_{j=1}^{k} c_j \Delta y_{t-j} + \varepsilon_t
  \label{eq:za}
\end{equation}
%
where $DU_t(\lambda){=}\mathbf{1}(t{>}T\lambda)$ and $DT_t(\lambda){=}(t{-}T\lambda)\mathbf{1}(t{>}T\lambda)$ are level and trend shift dummies at break fraction $\lambda$, chosen to minimise the $t$-statistic on $\hat{\alpha}$. Results are in Table~\ref{tab:za}. Bai-Perron PELT~\cite{pesaran2001} detects multiple breaks without pre-specifying their number (Table~\ref{tab:bp}). PELT confirms a single dominant break in the Underemployment Rate at 2022. GDP Growth, Inflation, and Services Employment Share all break at 2021---one year before the sovereign default---indicating that macro deterioration preceded the labour market response by approximately 4 quarters.
...
\textbf{ARDL Bounds Model.} The ARDL($p,q$) specification for the long-run relationship between underemployment $U_t$ and predictors $\mathbf{x}_t$ is:
%
\begin{equation}
  \Delta U_t = \alpha_0 + \sum_{i=1}^{p}\phi_i\,\Delta U_{t-i}
             + \sum_{j=0}^{q}\boldsymbol{\beta}_j'\,\Delta\mathbf{x}_{t-j}
             + \lambda_1 U_{t-1} + \boldsymbol{\lambda}_2'\mathbf{x}_{t-1} + \varepsilon_t
  \label{eq:ardl}
\end{equation}
%
where $\lambda_1$ and $\boldsymbol{\lambda}_2$ are the long-run level coefficients tested via Pesaran's $F$-bounds procedure~\cite{pesaran2001}. Due to the $n{=}10$ constraint, Johansen cointegration (singular matrix) and full ARDL ($12$ regressors $> 9$ d.f.) were infeasible at annual frequency. Estimation of Eq.~\eqref{eq:ardl} is deferred to the full quarterly dataset. A fallback OLS with HC3 robust standard errors is estimated ($R^2{=}0.767$, adj.~$R^2{=}0.475$), with diagnostics in Table~\ref{tab:granger} confirming well-behaved residuals. Granger causality tests use Bonferroni-corrected $\alpha{=}0.01$; no predictor clears the corrected threshold at $n{=}10$, as expected given low power below 15 observations.
...
The annual underemployment series is decomposed as:
%
\begin{equation}
  Y_t = T_t + S_t + R_t
  \label{eq:stl}
\end{equation}
%
where $T_t$ is the Loess-smoothed trend, $S_t$ the seasonal component, and $R_t$ the remainder~\cite{kpss1992}. STL is applied with robust fitting to minimise outlier influence. Figure~\ref{fig:stl} shows the decomposition. The trend component rises from 2.41\% (2015) to a peak of 2.67\% (2023) before partial recovery to 2.52\% in 2024, confirming that the crisis imparted a persistent upward shift rather than a transient spike. The seasonal component (range: $-0.24$ to $+0.19$~pp) is modest at annual frequency. The residual standard deviation of 0.121 indicates trend and seasonal components together explain the bulk of observed variation, with the 2022 LFS artefact (2.3\% reported rate) appearing as a clear negative residual spike.
...
XGBoost~\cite{chen2016xgboost} and Random Forest regressors are fitted on an 80/20 temporal split (time-order preserved). Exchange Rate is excluded from the ML feature set consistent with the VIF finding. XGBoost: \texttt{max\_depth=3}, \texttt{n\_estimators=100}, \texttt{learning\_rate=0.1}. SHAP~\cite{lundberg2017unified} decomposes the model prediction into additive feature contributions:
%
\begin{equation}
...
\end{equation}
%
where $\phi_0$ is the base (mean) prediction and $\phi_i$ is the Shapley value of feature $i$, satisfying efficiency, symmetry, and dummy axioms. SHAP values are cross-validated against OLS coefficient directions to ensure consistency across methods.
...
Bai-Perron PELT detects a single structural break in the underemployment rate at 2022, co-occurring with breaks in GDP Growth, Inflation, and Services Employment Share at 2021---one year prior. To quantify this elasticity shift, we estimate the interaction model:
%
\begin{equation}
...
\end{equation}
%
where $G_t$ is GDP Growth, $D_t{=}\mathbf{1}(t \geq 2022)$ is the crisis dummy, and $\beta_3$ captures the post-break amplification. The GDP interaction model achieves $R^2{=}0.495$ and the inflation model $R^2{=}0.594$. Figure~\ref{fig:interaction} shows the steepened post-2022 slope, confirming that a given unit of GDP contraction produces significantly greater underemployment pressure post-default than pre-crisis---validating RQ3.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth, height=3.2cm, keepaspectratio]{GDP_Interaction_Break.png}
  \caption{GDP Growth $\times$ Crisis interaction (Eq.~\ref{eq:interaction}). Blue: pre-break slope; red: post-2022 slope. Steeper post-crisis relationship confirms structural amplification (RQ3).}
  \label{fig:interaction}
\end{figure}

\subsection{Lagged Dependencies (RQ2, RQ4)}

Figure~\ref{fig:tlcc} shows GDP and inflation shocks lagging 2--3 quarters before realising in underemployment. This confirms delayed macro shock transmission consistent with labour market hysteresis, informs the XGBoost lag features, and provides directional support for long-run cointegration pending full ARDL estimation via Eq.~\eqref{eq:ardl} on the quarterly dataset.
...
Quarterly LFS micro-data reveals crisis underemployment peaks of 32.6\%---nine times the concurrent 3.8\% unemployment rate. Structural break tests (Zivot-Andrews Eq.~\ref{eq:za} and Bai-Perron PELT) confirm 2021--2022 as a statistically significant rupture in the underemployment--GDP--inflation system, with macro breaks leading the labour market break by one year. VIF analysis (Eq.~\ref{eq:vif}) identifies Exchange Rate as collinear with Inflation (VIF\,=\,12.28) and excludes it from long-run models. STL decomposition (Eq.~\ref{eq:stl}) confirms persistent post-crisis trend elevation. SHAP (Eq.~\ref{eq:shap}) establishes GDP contraction and hyperinflation as dominant drivers, with Youth LFPR and informal employment capturing the qualification-mismatch channel. These findings confirm aggregate unemployment is an inadequate crisis barometer for Sri Lanka.

\textbf{Limitations.} The annual dataset ($n{=}10$) rendered Johansen and full ARDL (Eq.~\ref{eq:ardl}) infeasible; results are directional pending quarterly expansion. Remittances and quarterly agricultural data are absent. The 2024 GDP rate is extrapolated; the 2022 LFS artefact complicates annual inference. STL seasonal estimates at annual frequency are indicative only.
