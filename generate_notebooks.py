import json

def create_notebook(title, feature_path, feature_col_name, color, output_name, desc_text):
    cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [f"# {title} vs Unemployment Analysis\n\nThis notebook provides an in-depth analysis of the relationship between {title} and Unemployment in Sri Lanka, matching the comprehensive exploratory and statistical depth seen in other macro-labour combinations."]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 1. Data Loading & Initial Exploration"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "import pandas as pd\n",
                "import numpy as np\n",
                "import matplotlib.pyplot as plt\n",
                "import seaborn as sns\n",
                "from scipy import stats\n\n",
                "plt.style.use('default')\n\n",
                "# Load main unemployment data (1991-2023)\n",
                "unemp_df = pd.read_csv('../labour/csv/API_SL.UEM.TOTL.ZS_DS2_en_csv_v2_93.csv', skiprows=4)\n",
                "unemp_sl = unemp_df[unemp_df['Country Name'] == 'Sri Lanka'].copy()\n",
                "years = [str(y) for y in range(1991, 2024)]\n",
                "unemp_sl = unemp_sl[['Country Name'] + years].melt(id_vars=['Country Name'], var_name='Year', value_name='Unemployment_Rate')\n",
                "unemp_sl['Year'] = pd.to_datetime(unemp_sl['Year'], format='%Y')\n",
                "unemp_sl.set_index('Year', inplace=True)\n",
                "unemp_sl.drop(columns=['Country Name'], inplace=True)\n\n",
                "print(\"Unemployment Data Info:\")\n",
                "print(unemp_sl.info())\n",
                "unemp_sl.head()"
            ]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "# Load Feature Data\n",
                f"feature_df = pd.read_csv('{feature_path}')\n",
                "feature_df = feature_df[['Year', 'Value']].copy()\n",
                f"feature_df.rename(columns={{'Value': '{feature_col_name}'}}, inplace=True)\n",
                "feature_df['Year'] = pd.to_datetime(feature_df['Year'].astype(str), format='%Y')\n",
                "feature_df.set_index('Year', inplace=True)\n\n",
                "print(f\"Feature Data Info:\")\n",
                "print(feature_df.info())\n",
                "feature_df.head()"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 2. Missing Values Analysis & Imputation"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "print(\"Missing values in Unemployment:\", unemp_sl['Unemployment_Rate'].isnull().sum())\n",
                f"print(\"Missing values in {feature_col_name}:\", feature_df['{feature_col_name}'].isnull().sum())\n\n",
                "# Applying Linear Interpolation (Best practice for macroeconomic time series)\n",
                "unemp_clean = unemp_sl.interpolate(method='linear').bfill().ffill()\n",
                f"feature_clean = feature_df.interpolate(method='linear').bfill().ffill()\n\n",
                "print(\"\\nPost-imputation completeness verified.\")"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 3. Data Alignment and Merging"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "# Inner join to guarantee alignment\n",
                "combined_df = unemp_clean.join(feature_clean, how='inner')\n\n",
                "print(f\"Combined Dataset: {len(combined_df)} records (from {combined_df.index.year.min()} to {combined_df.index.year.max()})\")\n",
                "combined_df.head()"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 4. Comprehensive Correlation Analysis"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "u_vals = combined_df['Unemployment_Rate'].values\n",
                f"f_vals = combined_df['{feature_col_name}'].values\n\n",
                "pearson_r, pearson_p = stats.pearsonr(f_vals, u_vals)\n",
                "spearman_r, spearman_p = stats.spearmanr(f_vals, u_vals)\n\n",
                "print(\"=\" * 60)\n",
                "print(\"CORRELATION STATISTICS\")\n",
                "print(\"=\" * 60)\n",
                "print(f\"Pearson (Linear):       r = {pearson_r:.4f}  | p-value = {pearson_p:.6f}\")\n",
                "print(f\"Spearman (Monotonic):   Rho = {spearman_r:.4f} | p-value = {spearman_p:.6f}\")\n",
                "print(\"=\" * 60)\n"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 5. Extensive Visualization (Structural Patterns)"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "fig, axes = plt.subplots(2, 2, figsize=(16, 12))\n\n",
                "# 1. Scatter with Regression Line\n",
                "sns.regplot(x=f_vals, y=u_vals, ax=axes[0, 0], scatter_kws={'alpha': 0.7, 's': 60}, line_kws={'color': 'red'}, color='{color}')\n",
                f"axes[0, 0].set_xlabel('{feature_col_name}', fontweight='bold')\n",
                "axes[0, 0].set_ylabel('Unemployment Rate (%)', fontweight='bold')\n",
                "axes[0, 0].set_title('Cross-Sectional Scatter & Linear Fit', fontweight='bold')\n\n",
                "# 2. KDE Density Plot (Bivariate)\n",
                f"sns.kdeplot(x=f_vals, y=u_vals, ax=axes[0, 1], cmap='Blues', fill=True, alpha=0.8)\n",
                f"axes[0, 1].set_xlabel('{feature_col_name}')\n",
                "axes[0, 1].set_ylabel('Unemployment Rate (%)')\n",
                "axes[0, 1].set_title('Bivariate Density Heatmap', fontweight='bold')\n\n",
                "# 3. Time Series Dynamics\n",
                "ax_time = axes[1, 0]\n",
                "ax_time.plot(combined_df.index.year, combined_df['Unemployment_Rate'], color='tab:blue', marker='o', label='Unemployment')\n",
                "ax_time.set_ylabel('Unemployment Rate (%)', color='tab:blue', fontweight='bold')\n",
                "ax_time.tick_params(axis='y', labelcolor='tab:blue')\n",
                "ax_twin = ax_time.twinx()\n",
                f"ax_twin.plot(combined_df.index.year, combined_df['{feature_col_name}'], color='{color}', marker='s', linestyle='--', label='{feature_col_name}')\n",
                f"ax_twin.set_ylabel('{feature_col_name}', color='{color}', fontweight='bold')\n",
                "ax_twin.tick_params(axis='y', labelcolor='{color}')\n",
                "ax_time.set_title('Historical Overlap (Dual Axis)', fontweight='bold')\n\n",
                "# 4. Boxplots for Data Distribution\n",
                "bp_data = pd.DataFrame({'Unemployment': (u_vals - u_vals.mean()) / u_vals.std(), \n",
                f"                      '{title} Context': (f_vals - f_vals.mean()) / f_vals.std()}})\n",
                "sns.boxplot(data=bp_data, ax=axes[1, 1], palette=['tab:blue', '{color}'])\n",
                "axes[1, 1].set_title('Standardized Distribution Variance', fontweight='bold')\n\n",
                "plt.tight_layout()\n",
                "plt.show()"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 6. Advanced Models: Linear Regression & Residuals"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "from sklearn.linear_model import LinearRegression\n",
                "from sklearn.metrics import r2_score, mean_squared_error\n\n",
                "X = f_vals.reshape(-1, 1)\n",
                "y = u_vals\n\n",
                "model = LinearRegression()\n",
                "model.fit(X, y)\n",
                "y_pred = model.predict(X)\n",
                "residuals = y - y_pred\n\n",
                "print(\"REGRESSION DIAGNOSTICS\")\n",
                "print(f\"R-squared Score: {r2_score(y, y_pred):.4f}\")\n",
                "print(f\"RMSE:            {np.sqrt(mean_squared_error(y, y_pred)):.4f}\")\n",
                "print(f\"Coefficient:     {model.coef_[0]:.4f}\")\n",
                "print(f\"Intercept:       {model.intercept_:.4f}\")\n"
            ]
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 7. Lagged Analysis (Time Delays in Macro Effects)"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
            "metadata": {},
            "source": [
                "print(\"LAG ANALYSIS: Macro-factors often take time to impact unemployment.\")\n",
                "print(\"=\" * 70)\n\n",
                "for lag in range(0, 5):\n",
                f"    shifted_feature = combined_df['{feature_col_name}'].shift(lag)\n",
                "    corr = combined_df['Unemployment_Rate'].corr(shifted_feature)\n",
                "    print(f\"Lag {lag} Years: correlation = {corr:.4f}\")\n",
                "\n",
                "print(\"\\nThis identifies how many years it takes for changes in this sector to penetrate the labour market.\")\n"
            ]
        }
    ]
    
    nb = {
        "cells": cells,
        "metadata": {
            "kernelspec": {
                "display_name": "Python 3",
                "language": "python",
                "name": "python3"
            }
        },
        "nbformat": 4,
        "nbformat_minor": 4
    }
    
    with open(output_name, 'w') as f:
        json.dump(nb, f, indent=1)

create_notebook(
    "Services Sector", 
    "../labour/finalized_csv/Employment_by_sector_(%)_sl_indicators/Employment in services (% of total employment) (modeled ILO estimate).csv",
    "Services_Sector_Share_%",
    "teal",
    "Data_Analysis/Services_Sector_vs_Unemployment.ipynb",
    "Services sector resilience acts as a major anchor against unemployment shocks."
)

create_notebook(
    "Youth LFPR", 
    "../labour/finalized_csv/Labor_force_participation_rate,_total_(% of total population ages 15-64)_(modeled ILO estimate)_sl_indicators/Labor force participation rate for ages 15-24, total (%) (modeled ILO estimate).csv",
    "Youth_LFPR_%",
    "#e74c3c",
    "Data_Analysis/Youth_LFPR_vs_Unemployment.ipynb",
    "Youth labor force entry provides massive signaling for general unemployment transitions."
)

create_notebook(
    "Agriculture Sector", 
    "../labour/finalized_csv/Employment_by_sector_(%)_sl_indicators/Employment in agriculture (% of total employment) (modeled ILO estimate).csv",
    "Agriculture_Sector_Share_%",
    "forestgreen",
    "Data_Analysis/Agriculture_Sector_vs_Unemployment.ipynb",
    "Agriculture absorbs rural labor slack and provides heavy causal inference bounds."
)
