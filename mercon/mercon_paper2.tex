...
limitations of~\cite{pabasara2025impact}: focus on
...
For Sri Lanka, prior work~\cite{pabasara2025impact} confirms Okun's
...
\end{equation}
%
\noindent where $\text{RER}_t$ is the real exchange rate at time $t$,
$e_t^{\text{nom}}$ is the nominal LKR/USD mid-rate,
$\text{CPI}_t^{\text{USA}}$ is the U.S.\ Consumer Price Index, and
$\text{CPI}_t^{\text{LKA}}$ is the Sri Lankan Consumer Price Index.
...
\end{multline}
%
\noindent where $\Delta U_t$ is the first difference of the
underemployment rate, $\alpha_0$ is the intercept, $\phi_i$ are
autoregressive coefficients over $p$ lags, $\boldsymbol{\beta}_j$ are
short-run coefficient vectors for the predictor vector
$\mathbf{x}_t$ over $q$ lags, $\lambda_1$ and
$\boldsymbol{\lambda}_2$ are the level (long-run) coefficients tested
via the bounds procedure, and $\varepsilon_t$ is the white-noise error
term. Here $\lambda_1$ and $\boldsymbol{\lambda}_2$ are tested via Pesaran's
...
\end{equation}
%
\noindent where $\mu$ is a constant, $\lambda$ is the speed-of-adjustment
coefficient (expected negative for convergence),
$\widehat{\text{ECT}}_{t-1}$ is the lagged error correction term from the
long-run equilibrium, $\psi_i$ are short-run autoregressive coefficients,
and $\boldsymbol{\gamma}_j$ capture the short-run dynamics of the
...
\end{equation}
%
\noindent where $\hat{f}(\mathbf{x})$ is the model prediction for input
$\mathbf{x}$, $\phi_0$ is the baseline (mean prediction), $\phi_i(x_i)$
is the Shapley value attributing the contribution of the $i$-th feature,
and $M$ is the total number of features. SHAP values are
...
\end{equation}
%
\noindent where $U_t$ is the underemployment rate, $G_t$ is GDP growth,
$D_t$ is a binary crisis dummy ($D_t{=}1$ for $t \geq 2022$),
$\beta_3$ captures the structural shift in macro elasticity post-crisis,
and $\varepsilon_t$ is the error term. This model achieves $R^2{=}0.495$ for GDP and $R^2{=}0.594$ for inflation
...
%% Acknowledgment removed for double-blind review
